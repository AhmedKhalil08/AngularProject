...
import { AllOrderModal } from '../../components/all-order-modal/all-order-modal';

@Component({
  selector: 'app-all-orders',
  imports: [FormsModule, CurrencyPipe, CommonModule, AllOrderModal],
...
export class AllOrders {
  ...
  totalSpent = computed(() => {
    return this.orders().reduce(
      (sum, order) => sum + (order.status === 'Delivered' ? order.totalAmount || 0 : 0),
      0,
    );
  });
